...
export default function Hero() {
  ...
  // 👇 불필요한 fetch, saveToFile 등 제거됨
  // 순수 프론트엔드만 남김 (오류 없음)

  return (
    <section
      id="hero"
      className="text-center py-24 bg-gradient-to-b from-white to-blue-50 min-h-screen flex flex-col justify-center items-center relative"
    >
      ...
      <p className="text-gray-600 mt-2 text-lg leading-relaxed">
        미국에서 20년 거주 후 한국에 정착하여<br />
        도시개발과 텍스트마이닝을 연구하는 석사과정생입니다.
      </p>

      <p className="text-gray-500 mt-6">
        “데이터로 도시의 마음을 읽는 사람”이 되고 싶어요.
      </p>

      <div className="mt-10">
        <Button onClick={() => setShowProjects(!showProjects)} size="lg">
          {showProjects ? "닫기" : "내 프로젝트 보기"}
        </Button>
      </div>

      {showProjects && (
        <div className="mt-8 bg-white/70 border rounded-xl shadow p-6 w-80">
          <p className="text-gray-700 mb-2 font-semibold">📊 진행 중인 연구</p>
          <ul className="text-gray-600 text-sm space-y-1">
            <li>• 구글맵 리뷰 기반 도시공원 인식 비교</li>
            <li>• 내국인 vs 외국인의 도시 이미지 분석</li>
            <li>• 텍스트마이닝과 사회연결망 분석 활용</li>
          </ul>
        </div>
      )}
